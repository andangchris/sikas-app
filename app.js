...
if (iuranKas > 0) {
  ...
  // Sembunyikan div Kas jika iuranKas = 0
  if (kasEl) kasEl.innerHTML = "";
}
...
if (jmlKas) {
  if (iuranKas > 0) {
    jmlKas.max = kasList.length;
    jmlKas.value = 0;
    jmlKas.parentElement.style.display = "block"; // tampilkan input
  } else {
    jmlKas.max = 0;
    jmlKas.value = 0;
    jmlKas.parentElement.style.display = "none"; // sembunyikan input
  }
}
...
function totalIuranBulanan(p) {
  ...
  const ikutRmd = p?.ikut_rmd === true || String(p?.ikut_rmd || "").toLowerCase() === "true" || String(p?.ikut_rmd || "").toLowerCase() === "ya" || String(p?.ikut_rmd || "").toLowerCase() === "y";
  const rmd = ikutRmd ? Number(p?.iuran_rmd || 0) : 0;
  return (kas > 0 ? kas : 0) + (rmd > 0 ? rmd : 0);
}
